...
import { 
  FaPlus, FaHistory, FaMusic, 
  FaChevronRight, FaDotCircle, FaSpotify, FaBolt, FaListUl, FaShieldAlt, FaChartLine 
} from 'react-icons/fa';
...
export default function HomePage() {
  ...
  const activeSessions = [
    { id: 1, name: "Morning Brew Café", status: "Active", songs: 12, listeners: 48 },   
  ];
 
  const pastSessions = [
    { id: 2, name: "Friday Jazz Night", date: "Jan 16, 2026", totalSongs: 84 },
    { id: 3, name: "Acoustic Sunday", date: "Jan 11, 2026", totalSongs: 52 },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FEF3EB] text-[#5C4033] relative overflow-x-hidden">
      {/* 1. Background Visuals (Matching Login/Register) */}
      ...
      {/* 2. Main Content - Added pt-28 to clear the fixed navbar */}
      <main className="relative z-10 max-w-7xl mx-auto pt-28 pb-12 px-6 lg:px-16">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div>
            <h1 className="text-4xl font-black tracking-tighter text-[#5C4033]">Host Dashboard</h1>
            <p className="text-[#5C4033]/60 font-medium mt-1">Manage your venue's live music sessions.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <section className="animate-in fade-in slide-in-from-left-4 duration-700 delay-100">
              ...
              {activeSessions.length > 0 ? (
                activeSessions.map((session) => (
                  <div key={session.id} className="bg-white/70 backdrop-blur-md border border-white p-6 rounded-[2rem] shadow-sm hover:shadow-xl hover:shadow-[#5C4033]/5 transition-all group">
                    <div className="flex items-center justify-between flex-wrap gap-4">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-[#5C4033] group-hover:text-[#E07A3D] transition-colors">{session.name}</h3>
                ...
                <div className="p-12 border-2 border-dashed border-[#5C4033]/10 rounded-[2.5rem] text-center bg-white/30">
                  <p className="text-[#5C4033]/40 font-bold">No active sessions. Start one to get the music moving!</p>
                </div>
              )}
            </section>
            ...
            <section className="animate-in fade-in slide-in-from-left-4 duration-700 delay-200">
              <h2 className="text-sm font-black uppercase tracking-[0.2em] text-[#5C4033]/40 mb-6">Recent History</h2>
                ...
                {pastSessions.map((session, index) => (
                  <div 
                    ...
                    className={`p-6 flex items-center justify-between hover:bg-white/60 transition-colors cursor-pointer ${
                      index !== pastSessions.length - 1 ? 'border-b border-[#5C4033]/5' : ''
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-[#5C4033]">{session.name}</p>
                        <p className="text-[10px] font-black text-[#5C4033]/30 uppercase tracking-widest">{session.date}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-[#5C4033]/80">{session.totalSongs} songs played</p>
                      <button className="text-[#E07A3D] text-[10px] font-black uppercase tracking-widest hover:underline mt-1">View Analytics</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-[#3d2b22] text-white p-8 rounded-[2.5rem] shadow-2xl shadow-[#3d2b22]/20 relative overflow-hidden group">
              <div className="relative z-10">
                ...
                <p className="text-white/60 text-sm leading-relaxed mb-8 font-medium">
                  Keep your "Auto-accept" on during peak hours to focus on your customers while the music flows.
                </p>
              ...
              <FaMusic size={120} className="absolute -bottom-4 -right-4 text-white/5 rotate-12 group-hover:rotate-0 transition-transform duration-700" />
            </div >

            {/* Quick Management Card */}
            <div className="bg-white/70 backdrop-blur-md p-8 rounded-[2.5rem] border border-white shadow-sm">
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#5C4033]/40 mb-6">Quick Management</h4>
              <nav className="space-y-1">
                {[
                  ...
                  <button key={i} className="w-full flex items-center justify-between p-3.5 rounded-xl hover:bg-[#E07A3D]/5 text-[#5C4033] font-bold text-sm transition-all group">
                    <span className="flex items-center gap-3">
                      <span className="text-[#5C4033]/20 group-hover:text-[#E07A3D] transition-colors">{item.icon}</span>
                      {item.label}
                    </span>
